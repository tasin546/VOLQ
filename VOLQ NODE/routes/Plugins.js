...
function safePath(base, ...segments) {
    ...
    if (fullPath !== resolvedBase && !fullPath.startsWith(resolvedBase + path.sep)) throw new Error('Path traversal detected');
    return fullPath;
}
...
router.get('/:id', async (req, res) => {
    const { id } = req.params;
    const pluginsPath = safePath(VOLUMES_DIR, id, 'plugins');
    try {
