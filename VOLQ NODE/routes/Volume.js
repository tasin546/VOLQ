...
const VOLUMES_DIR = path.resolve(__dirname, "../volumes");

function getVolumePath(volumeId) {
  if (!volumeId || typeof volumeId !== "string") {
    throw new Error("Invalid volume ID");
  }
  return safePath(VOLUMES_DIR, volumeId);
}
...
router.get("/:id/files", async (req, res) => {
  ...
  const volumePath = getVolumePath(volumeId);
...
function unzipFile(archivePath, destinationPath) {
  return new Promise((resolve, reject) => {
    execFile("unzip", ["-o", archivePath, "-d", destinationPath], (error, stdout, stderr) => {
      if (error) {
        ...
        resolve(`Successfully unzipped ${path.basename(archivePath)}`);
      }
    });
  });
}
...
router.post("/:id/files/unzip/:file", async (req, res) => {
  ...
  const subPath = req.query.path || "";
  const volumePath = getVolumePath(id);
  ...
  try {
    const fullPath = safePath(volumePath, subPath);
    const archivePath = safePath(volumePath, file);

    const result = await unzipFile(archivePath, fullPath);
...
router.post("/:id/files/rename/:filename/:newfilename", async (req, res) => {
  ...
  try {
    const oldPath = safePath(volumePath, subPath, filename);
    const newPath = safePath(volumePath, subPath, newfilename);
...
router.post("/:id/files/upload", upload.array("files"), async (req, res) => {
  ...
  try {
    ...
    await Promise.all(
      req.files.map((file) => {
        const destPath = safePath(volumePath, subPath, file.originalname);
...
router.post("/:id/files/create/:filename", async (req, res) => {
  ...
  try {
    ...
    const fullPath = safePath(volumePath, subPath, filename);
...
router.post("/:id/folders/create/:foldername", async (req, res) => {
  ...
  try {
    // Ensure the path is safe and resolve it to an absolute path
    const targetFolderPath = safePath(volumePath, subPath, foldername);
...
router.post("/:id/folders/rename/:foldername/:newfoldername", async (req, res) => {
  ...
  try {
    const oldPath = safePath(volumePath, subPath, foldername);
    const newPath = safePath(volumePath, subPath, newfoldername);
...
router.delete("/:id/files/delete/:filename", async (req, res) => {
  ...
  try {
    const filePath = safePath(volumePath, subPath, filename);
